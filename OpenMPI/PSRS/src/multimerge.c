...
};mmdata_defult={0,0,0};//(int st=0, int id=0, int stv = 0):stindex(st),index(id),stvalue(stv){}



// comparison operator
bool lt( const struct mmdata* One, const struct mmdata* Two)
{
			   ...
			   int newArray[], const int newArrayLength)
{
	...
	while (!priorities.empty() && (newArrayindex<newArrayLength))
	{
		...
		struct mmdata xxx = priorities.top();
		priorities.pop();
