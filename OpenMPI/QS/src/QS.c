...
/////////////////////////////////
static int
gather_values(mdata *md, int nelements) {
    int i;
    ...
    if (md->rank == 0) {
        ...
        md->buf = (int*) malloc( nelements * sizeof(int) );
        /* because the lower the rank is the higher number we have */
#ifdef DEBUG
        debug_tables(md);
#endif
        if ( md->size > 0 ) {
            memcpy( &md->buf[nelements - md->size], md->set, md->size * sizeof(int));
            offset = nelements - md->size;
        } else offset = nelements;
...
/////////////////////////////////
static int
distribute_values(mdata *md, int nelements) {
    int i;
    int *buf;
    int size;
    int offset;
    ...
    fflush(stdout);
#endif
    size = floor( nelements / md->numtasks );
    offset = size + ( nelements - md->numtasks * size );
    if ( md->rank == 0 )
        md->size = offset;
...
/////////////////////////////////
static int
gen_set(mdata *md,int seed, int nelements) {
    int i;
    ...
    if ( md->rank == 0 ) {
        md->set = (int *) malloc(nelements * sizeof(int));
        md->size = nelements;
//        srand(time(NULL));
...
//    }
        initialize(md->set, seed, nelements);
    }
...
#endif
    md.gh = md.numtasks;
    gen_set(&md,seed,nelements);

#ifdef TEST
    MPI_Barrier(MPI_COMM_WORLD);
    etime = - MPI_Wtime();
#endif
    distribute_values(&md,nelements);
    pqsort(&md);
    gather_values(&md,nelements);
#ifdef TEST
    etime += MPI_Wtime();
    if ( md.rank == 0 ) {
        if ( sort_ok(&md) )
                printf("OpenMPI-QS,%d,%d,%f\n",md.numtasks,nelements,etime);
//            printf("0: Elapsed time: %.3f [s], numproc: %d, datasize: %d, status: OK\n", etime, md.numtasks, nelements);
        else
            printf("0: Elapsed time: %.3f [s], numproc: %d, datasize: %d, status: FAILED\n", etime, md.numtasks, nelements);
    }
#endif
//    testit(md.set,1000000);
    free_memory(&md);
